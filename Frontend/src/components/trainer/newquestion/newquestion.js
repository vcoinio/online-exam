...
class NewQuestion extends Component {
    ...
    Customalert = () => {
        Modal.confirm({
            ...
            content: this.props.t('empty option can not be set as answer'),
            okText: this.props.t('I understand'),
            cancelText: null,
        });
    }
    ...
    handleSubmit = e => {
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            ...
            if (!err) {
                ...
                if (f) {
                    if (!ans) {
                        Alert('warning', this.props.t('Warning!'), this.props.t('There must be atleast one right answer'));
                    }
                    else {
                        this.setState({
                            adding: true
                    ...
                    Alert('warning', this.props.t('Warning!'), this.props.t('Please fill all the options'));
                }
            }
        });
    };
                ...
                <div className="register-trainer-form-body">
                    <Form onSubmit={this.handleSubmit}>
                        <div>
                            <Row>
                                <Col span={8}>
                                    <Form.Item label={this.props.t('Subject')} hasFeedback>
                                        {getFieldDecorator('subject', {
                                            rules: [{ required: true, message: this.props.t('Please select any subject!') }],
                                        })(
                                            <Select
                                                showSearch
                                                style={{ width: '100%' }}
                                                placeholder={this.props.t('Select a subject')}
                                                optionFilterProp="s"
                                            >
                                                {
                            ...
                            <Row>
                                <Col span={18}>
                                    <Form.Item label={this.props.t('Question')} hasFeedback>
                                        {getFieldDecorator('questionbody', {
                                            rules: [{ required: true, message: this.props.t('Please type question!') }],
                                        })(
                                            <TextArea rows={5} />
                                        )}
                                    </Form.Item>
                            ...
                            <Row>
                                <Col span={18}>
                                    <Form.Item label={this.props.t('Explanation')} hasFeedback>
                                        {getFieldDecorator('explanation', {
                                            rules: [{ required: true, message: this.props.t('Please type Explanation for the answers!') }],
                                        })(
                                ...
                                <Col offset={2} span={4}>
                                    <Form.Item label={this.props.t('Weightage')} hasFeedback>
                                        {getFieldDecorator('waitage', {
                                            rules: [{ required: true, message: this.props.t('Please enter the marks') }],
                                        })(
                                        ...
                                        return (
                                            <Row key={i} className="">
                                                <Col offset={1} span={13}>
                                                    <Form.Item label={`${this.props.t('option')}${i + 1}`}>
                                                ...
                                                <Col offset={2} span={6} style={{ textAlign: 'center' }}>
                                                    <Form.Item label={`${this.props.t('Option Image')} ${i + 1}`}>
                            ...
                            <Row>
                                <Col span={12}>
                                    {this.state.fifthoptioAddButtonVisible ? <Button type="primary" onClick={(e) => this.addfifthOption(e)}>{this.props.t('Add 5th option')}</Button> : null}
                                </Col>
                            </Row>
                            <Row>
...
const NewQuestionForm = Form.create({ name: 'newQuestion' })(withTranslation()(NewQuestion));
