...
class NewTrainer extends Component {
    ...
    compareToFirstPassword = (rule, value, callback) => {
        ...
        if (value && value !== form.getFieldValue('password')) {
            callback(this.props.t('passwords are not same !'));
        } else {
            callback();
        }
    };
    ...
    render() {
        ...
        const prefixSelector = getFieldDecorator('prefix', {
            ...
            rules: [{ required: true, message: this.props.t('Please enter contact no prefix') }],
        })(
            ...
            <div className="register-trainer-form" >
                <div className="register-trainer-form-body">
                    <Form onSubmit={this.handleSubmit}>
                        <Form.Item label={this.props.t('Name')} hasFeedback className="input-admin-trainer">
                            {getFieldDecorator('name', {
                                initialValue: this.props.admin.trainerdetails.name,
                                rules: [{ required: true, message: this.props.t('Please input your name!'), whitespace: true }],
                            })(<Input />)}
                        </Form.Item>

                        {!this.props.admin.trainerId ? <Form.Item label={this.props.t('E-mail')} hasFeedback className="input-admin-trainer">
                        ...
                        <Form.Item label={this.props.t('Phone Number')} className="input-admin-trainer">
                            {getFieldDecorator('contact', {
                                initialValue: this.props.admin.trainerdetails.contact,
                                rules: [
                                    {
                                        required: true,
                                        message: this.props.t('Please input your phone number!')
                                    },
                                    {
                                        len: 10,
                                        message: this.props.t('Contact number must be 10 digit long')
                                    }],
                            })(<Input addonBefore={prefixSelector} min={10} max={10} />)}
                        </Form.Item>

                        {!this.props.admin.trainerId ? <div><Form.Item label={this.props.t('Password')} hasFeedback className="input-admin-trainer">
                            {getFieldDecorator('password', {
                                initialValue: this.props.admin.trainerdetails.password,
                                rules: [
                                    {
                                        required: true,
                                        message: this.props.t('Please input your password!'),
                                    },
                                    {
                            ...
                            <Form.Item label={this.props.t('Confirm Password')} hasFeedback className="input-admin-trainer">
                                {getFieldDecorator('confirm', {
                                    initialValue: this.props.admin.trainerdetails.confirmpassword,
                                    rules: [
                                        {
                                            required: true,
                                            message: this.props.t('Please confirm your password!'),
                                        },
                                        {
...
const NewTrainerForm = Form.create({ name: 'register' })(withTranslation()(NewTrainer));
